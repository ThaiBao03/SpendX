...
export function AddTransactionModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createTransaction({
        ...
        amount: numAmount,
        type,
        date: new Date().toISOString(), // luôn lấy thời điểm hiện tại
      });
      onSuccess?.();
      onClose();
    } catch (err) {
  ...
  // Format số tiền khi nhập (thêm dấu chấm ngàn)
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[13px] font-semibold text-slate-400">
                đ
              </span>
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full flex items-center justify-center gap-2 py-3 rounded-2xl text-[14px] font-bold text-white transition-all shadow-lg
              ${type === "income"
